...
// ============================================
// NEW ACTION: GET USER INFO BY ID NUMBER
// ============================================
if ($action === 'get_user_info') {
    ...
    $stmt = $conn->prepare("SELECT first_name, last_name, email, status FROM users WHERE id_number = :id_number");
    ...
    if ($user['status'] !== 'approved') {
        echo json_encode(['status' => 'error', 'message' => 'Your account is currently ' . $user['status'] . '. Please contact an administrator (admin or superadmin).']);
        exit();
    }
    ...
    $maskedEmail = substr($name, 0, 1) . str_repeat("*", max(0, strlen($name) - 1)) . $domain;
...
if ($action === 'send_otp') {
    ...
    $stmt = $conn->prepare("SELECT id, id_number, email, first_name, last_name, role, status FROM users WHERE id_number = :id_number");
    $stmt->execute(['id_number' => $id_number]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    if ($user['status'] !== 'approved') {
        $response['message'] = 'Your account is currently ' . $user['status'] . '. Please contact an administrator (admin or superadmin).';
        echo json_encode($response);
        exit();
    }
...
if ($action === 'reset_password') {
    ...
    // Enforce new complex password requirements (12-15 chars, Uppercase, Lowercase, Number, Special)
    ...
    if (!$hasUpper || !$hasLower || !$hasNumber || !$hasSpecial || $length < 12 || $length > 15) {
        $response['message'] = 'Password must be 12-15 characters long and include uppercase, lowercase, numbers, and special characters.';
        echo json_encode($response);
        exit();
    }
